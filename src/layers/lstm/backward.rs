...
impl LstmLayer {
    ...
    /// Backward pass with incoming BPTT state gradients.
    ///
    /// Performs the same backward computation as `backward_impl`, but additionally
    /// incorporates incoming state gradients from the *next* time step (`dh_next`,
    /// `dc_next`).  These are added element-wise to the gradient w.r.t. `h_t` and
    /// `c_t` before propagating further back through the gates.
    ///
    /// After this call, the results of propagating to the previous time step are
    /// stored in `self.grad_h_prev` and `self.grad_c_prev`, which the public
    /// `backward_bptt` method returns as a tuple.
    ///
    /// # Arguments
    ///
    /// * `input` - Input data (batch_size × input_size)
    ...
    /// # Panics
    ///
    /// Panics if `batch_size == 0` or if any slice length does not match the expected size.
    pub(super) fn backward_bptt_impl(
        &self,
        input: &[f32],
        grad_output: &[f32],
        grad_input: &mut [f32],
        ...
        if batch_size == 0 {
            panic!("batch_size cannot be zero in LSTM::backward_bptt");
        }
        ...
        assert_eq!(
            dh_next.len(),
            self.hidden_size,
            "dh_next size mismatch in backward_bptt (expected hidden_size)"
        );
        assert_eq!(
            dc_next.len(),
            self.hidden_size,
            "dc_next size mismatch in backward_bptt (expected hidden_size)"
        );
        ...
        // Gradient w.r.t. h_t from output projection: grad_h = grad_output × W_hy^T
        // Then add incoming BPTT hidden state gradient dh_next (broadcast over batch).
        // Total: grad_h_total = grad_h_from_output + dh_next
        let mut grad_h = vec![0.0f32; batch_size * self.hidden_size];
        unsafe {
            sgemm(
                Layout::RowMajor,
        ...
        // Add incoming hidden state gradient from next time step (broadcast over all batch rows)
        for b in 0..batch_size {
            for h in 0..self.hidden_size {
        ...
        // ========== Step 3: Backprop through tanh to get grad w.r.t. c_t ==========
        // tanh'(c_t) = 1 - tanh(c_t)^2
        // Then add incoming BPTT cell state gradient dc_next (broadcast over batch).
        // Total: grad_c_total = grad_c_from_tanh + dc_next
        let mut grad_cell_state = vec![0.0f32; batch_size * self.hidden_size];
        for i in 0..grad_cell_state.len() {
            let tanh_val = cell_tanh_batch[i];
            grad_cell_state[i] = grad_cell_tanh[i] * (1.0 - tanh_val * tanh_val);
        ...
        // Add incoming cell state gradient from next time step (broadcast over all batch rows)
        for b in 0..batch_size {
            for h in 0..self.hidden_size {
        ...
        // ========== Step 4: Backprop through output gate (o_t = σ(...)) ==========
