...
use rust_neural_networks::utils::lr_scheduler::{
    create_scheduler_from_config, CosineAnnealing, ExponentialDecay, LRScheduler, StepDecay,
};
use std::io::Write;
...
mod from_config_tests {
    ...
    #[test]
    fn test_step_decay_from_config() {
        let config = load_config("config/mnist_mlp_step.json").unwrap();
        let mut sched = StepDecay::from_config(0.1, &config).expect("valid step_size");
        assert_eq!(sched.get_lr(), 0.1);
        sched.step();
        sched.step();
        sched.step();
        assert_relative_eq!(sched.get_lr(), 0.05, epsilon = 1e-6);
    }

    #[test]
    fn test_exponential_decay_from_config() {
        let config = load_config("config/mnist_mlp_exponential.json").unwrap();
        let mut sched = ExponentialDecay::from_config(0.1, &config);
        sched.step();
        assert_relative_eq!(sched.get_lr(), 0.095, epsilon = 1e-6);
    }

    #[test]
    fn test_cosine_annealing_from_config() {
        let config = load_config("config/mnist_mlp_cosine.json").unwrap();
        let mut sched = CosineAnnealing::from_config(0.1, &config);
        assert_eq!(sched.get_lr(), 0.1);
        sched.step();
...
// ============================================================================
// create_scheduler_from_config Tests
// ============================================================================

#[cfg(test)]
mod create_scheduler_from_config_tests {
    use super::*;

    #[test]
    fn test_create_scheduler_no_config_path() {
        let mut sched = create_scheduler_from_config(0.1, 5, None);
        let lr = sched.get_lr();
        sched.step();
        assert_eq!(sched.get_lr(), lr);
    }

    #[test]
    fn test_create_scheduler_step_decay_valid() {
        let config_json = r#"{
  ...
  "gamma": 0.5
        ...
        let mut sched = create_scheduler_from_config(0.1, 5, Some(temp.path().to_str().unwrap()));
        assert_eq!(sched.get_lr(), 0.1);
        sched.step();
        assert_eq!(sched.get_lr(), 0.1);
        sched.step();
    ...
    #[test]
    fn test_create_scheduler_step_decay_invalid_fallback() {
        let config_json = r#"{
  "scheduler_type": "step_decay",
  "step_size": 0,
  "gamma": 0.5
    ...
    #[test]
    fn test_create_scheduler_exponential() {
        let config_json = r#"{
  ...
  "decay_rate": 0.9
        ...
        let mut sched = create_scheduler_from_config(0.1, 5, Some(temp.path().to_str().unwrap()));
        sched.step();
        assert_relative_eq!(sched.get_lr(), 0.09, epsilon = 1e-6);
    }

    #[test]
    fn test_create_scheduler_cosine_invalid_min_lr() {
        let config_json = r#"{
  ...
  "T_max": 5
    ...
    #[test]
    fn test_create_scheduler_cosine_invalid_t_max() {
        let config_json = r#"{
  "scheduler_type": "cosine_annealing",
  "min_lr": 0.0,
  "T_max": 0
  ...
  "min_lr": 0.0
    ...
    #[test]
    fn test_create_scheduler_constant() {
        let config_json = r#"{
  "scheduler_type": "constant"
    ...
    #[test]
    fn test_create_scheduler_unknown_type() {
        let config_json = r#"{
  "scheduler_type": "mystery"
}"#;
        let temp = write_temp_config(config_json);
        let mut sched = create_scheduler_from_config(0.1, 5, Some(temp.path().to_str().unwrap()));
        let lr = sched.get_lr();
    ...
    #[test]
    fn test_create_scheduler_load_error_fallbacks() {
        let mut sched = create_scheduler_from_config(0.1, 5, Some("nonexistent.json"));
        let lr = sched.get_lr();
        sched.step();
        assert_eq!(sched.get_lr(), lr);
    }
